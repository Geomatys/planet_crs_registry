...
def get_apache_sis_path(
    ...
    try:
        ...
        raise Exception(f"Error downloading Apache SIS: {e}")
    except (zipfile.BadZipFile, zipfile.LargeZipFile) as e:
        raise Exception(f"Error extracting Apache SIS zip file: {e}")
    except FileNotFoundError as e:
        raise Exception(f"Error finding required files or directories: {e}")
...
def generate_all_gml_files(
    apache_jvm: ApacheJVM, output_directory: str
) -> None:
...
def main() -> None:
    ...
    apache_jvm = ApacheJVM(apache_libs)

    apache_jvm.start_jvm()
    try:
        generate_all_gml_files(apache_jvm, "gml")
    finally:
        apache_jvm.stop_jvm()
        if temp_dir_flag:
